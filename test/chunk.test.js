...
describe('A BaseChunk instance', function () {
  ...
  it('throws on anything but a string', muted(muter, function () {
    expect(() => new Chunk(42).chunk).to.throw('Not a plain chunk');
    expect(() => new Chunk({}).chunk).to.throw('Not a plain chunk');
    expect(() => new Chunk([]).chunk).to.throw('Not a plain chunk');
  }));

  it('throws on non plain string', muted(muter, function () {
    expect(() => new Chunk('*').chunk).to.throw('Not a plain chunk');
    expect(() => new Chunk('a?').chunk).to.throw('Not a plain chunk');
    expect(() => new Chunk('!abc').chunk).to.throw('Not a plain chunk');
    expect(() => new Chunk('a/b/c').chunk).to.throw('Not a plain chunk');
  }));
});
